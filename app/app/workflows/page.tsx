...
function workflowActionLabels(actions: unknown) {
  ...
  return actions
    .filter(
      (action): action is { type: string; label: string; email?: string } =>
        Boolean(action) &&
    ...
    .map(
      (action) => `${action.label}${action.email ? ` -> ${action.email}` : ""}`
    )
}
...
function workflowRunSteps(output: unknown) {
  ...
  return steps
    .filter(
      (step): step is { tool: string; status: string; summary: string } =>
        Boolean(step) &&
        typeof step === "object" &&
        typeof (step as { tool?: unknown }).tool === "string" &&
        typeof (step as { summary?: unknown }).summary === "string"
    )
    .map((step, index) => ({
      key: `${step.tool}-${index}`,
      tool: step.tool,
            ...
            {workflows.map((workflow) => (
              ...
              >
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Last run: {workflow.runs[0]?.status ?? "never"}
                </p>
                  ...
                  <div className="mt-3 rounded-md border border-slate-200 bg-white p-3 text-xs leading-5 text-slate-600">
                    ...
                    {workflowRunSteps(workflow.runs[0].output).length ? (
                      <div className="mt-3 space-y-2">
                        {workflowRunSteps(workflow.runs[0].output).map(
                          (step) => (
                            <div
                              key={step.key}
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      {workflowOutputValue(
                        ...
                        <Link
                          href="/app/crm"
                          className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-blue-900 transition hover:border-blue-200 hover:bg-blue-100"
                        >
                      ...
                      {workflowOutputValue(
                        ...
                        <Link
                          href="/app/tasks"
                          className="rounded-md border border-emerald-100 bg-emerald-50 px-3 py-2 text-emerald-900 transition hover:border-emerald-200 hover:bg-emerald-100"
                        >
                          <span className="block font-semibold">
                            Task added
                          </span>
                      ...
                      {workflowOutputValue(
                        ...
                        <Link
                          href="/app/support"
                          className="rounded-md border border-amber-100 bg-amber-50 px-3 py-2 text-amber-900 transition hover:border-amber-200 hover:bg-amber-100"
                        >
                          <span className="block font-semibold">
                            Support updated
                          </span>
