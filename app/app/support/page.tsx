  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <div className="op-panel p-5">
        ...
        <BulkDeleteProvider
          endpoint="/api/support/tickets/bulk"
          label="tickets"
        >
          <BulkDeleteToolbar />
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DeleteResourceButton
                      endpoint={`/api/support/tickets/${ticket.id}`}
                      label="ticket"
                    />
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-2">
                  {ticket.messages.slice(-3).map((message) => (
                    <div
                      key={message.id}
                      className={`rounded-md border px-3 py-2 text-xs leading-5 ${message.fromAgent ? "border-blue-100 bg-blue-50 text-blue-900" : "border-slate-200 bg-white text-slate-600"}`}
                    >
                      <p className="mb-1 font-semibold">
                        {message.fromAgent ? "OpsPilot reply" : "Customer"}
                      </p>
                      {message.body}
                    </div>
                  ))}
                ...
                !ticket.messages.some(
                  (message) =>
                    message.fromAgent && message.body === ticket.aiDraft
                ) ? (
                  <SendTicketDraftForm
                    ticketId={ticket.id}
